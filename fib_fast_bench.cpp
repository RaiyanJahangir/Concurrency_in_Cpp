...
static coro::DetachedTask fib_fast_task_coro(unsigned n,
                                             size_t idx,
...
static double fib_coroutine_batch(ThreadPool& pool,
    ...
    for (size_t i = 0; i < tasks; ++i) {
        fib_fast_task_coro(n, i, out, sched, done, tasks, m, cv, ep, ep_m);
    }
...
static void usage(const char* prog) {
    std::cerr
        << "Usage:\n  " << prog
        << " <pool: classic|elastic|ws|advws|coro> <fib_n> <threads> <warmup> <reps> [tasks]\n\n"
        << "Examples:\n"
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (pool_kind == "classic") {
            ...
            for (int i = 0; i < warmup; ++i) {
                uint64_t discard = 0;
                (void)fib_coroutine_batch(pool, fib_n, tasks, discard);
            }
            for (int r = 0; r < reps; ++r) {
                uint64_t checksum = 0;
                const double t = fib_coroutine_batch(pool, fib_n, tasks, checksum);
                best = std::min(best, t);
                sum += t;
                last_checksum = checksum;
                std::cout << "Run " << r << ": " << t << " s\n";
